...
@if(request()->segment(6) == 'box_circles' || request()->segment(6) == 'overview_sliders')
<script>
    $("select[name='section_id']").change(function(){
        var section_id = $(this).val();
        $.ajax( {
            type: "GET",
            url: "{{ url('admin-cp/section/box_circles/placement') }}"+'/'+section_id,
            success: function( result ) {
                $("select[name='placement']").html(result);
            }
        } );
    });
</script>
